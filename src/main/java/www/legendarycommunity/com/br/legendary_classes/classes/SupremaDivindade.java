...
public class SupremaDivindade implements Listener {

    private final Map<Integer, Integer> levels;
    private final Legendary_classes plugin;

    public SupremaDivindade(Legendary_classes plugin) {
    ...
    public boolean IsSupremaDivindade(Player player) {
        PlayerClassData data = plugin.getPlayerData(player.getUniqueId());
        return data != null && "SupremaDivindade".equalsIgnoreCase(data.getClassName());
    }
